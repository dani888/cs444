...
void process0()
{
  ...
  kprintf("\nSHUTDOWN\n");
  sti();
  kprintf("\nGoing through tests: for main1, main2, main3");
  kprintf("\nconfirmation of the Scheduler working\n\n");
  for (i = 1; i < NPROC; i++) {
    kprintf("\nexit code : %d: %d\n", i, proctab[i].p_exitval);
    kprintf("number of chars enqueued : %d\n", proctab[i].enqueCount);
    kprintf("ticks in process : %d\n", proctab[i].proc_tick);;
  }
  for (a = 0; a < 4; a++)
...
void init_proctab()
{
  ...
  for (i = 0; i < NPROC; i++) {
    proctab[i].proc_quantum = Quantumax;
    proctab[i].proc_tick = 0; // for timetest
    proctab[i].p_savedregs[SAVED_EBP] = 0;
    proctab[i].enqueCount = 0; // for tty
    proctab[i].p_savedregs[SAVED_EFLAGS] = EFLAGS_IF; /* make IF=1 */
    proctab[i].p_status = RUN;
    proctab[i].pNumer = i; // for timetest
...
void shutdown()
{
  shutdown_ticks(); // shhut off ticks
